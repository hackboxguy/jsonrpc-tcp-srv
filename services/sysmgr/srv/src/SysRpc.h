...
public:
  SysRpc(std::string rpcName, int myIndex, bool emu, bool log,
  ...
  int json_to_bin_download_file(JsonDataCommObj *pReq,
                                EJSON_SYSMGR_RPC_TYPES command);
  int bin_to_json_download_file(JsonDataCommObj *pReq,
                                EJSON_SYSMGR_RPC_TYPES command);
  int process_download_file(JsonDataCommObj *pReq,
                            EJSON_SYSMGR_RPC_TYPES command,
                            ADJsonRpcMgrProducer *pObj);
  RPC_SRV_RESULT
  process_async_download_file(SYSMGR_DOWNLOAD_FILE_PACKET *pPacket,
                              EJSON_SYSMGR_RPC_TYPES command);
  ...
  int process_subscribe_events(JsonDataCommObj *pReq,
                               ADJsonRpcMgrProducer *pObj);

  // int json_to_bin_run_shellcmdtrig(JsonDataCommObj* pReq);
  // int bin_to_json_run_shellcmdtrig(JsonDataCommObj* pReq);
  // int process_run_shellcmdtrig(JsonDataCommObj* pReq,ADJsonRpcMgrProducer*
  // pObj); RPC_SRV_RESULT
  // process_async_run_shellcmdtrig(SYSMGR_SHELLCMD_PACKET* pPacket);
};
/* ------------------------------------------------------------------------- */
