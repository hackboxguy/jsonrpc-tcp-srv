...
#define LOG_MSG_MATCH_STRING                                                   \
  "BRBOX" // for filtering brbox-services specific messages
...
class LogHandler : public ADChainConsumer {
  ...
  RPC_SRV_RESULT read_log_message_count(char *log_filepath, char *match_string,
                                        int *TotalMessages);
  ...
  int grep_log_message_line(char *log_filepath, char *match_string, int line,
                            char *message);
  int push_log_message(int line, char *message, ADGenericChain *pChain);
  int update_logmsg_chain(char *log_filepath, char *match_string,
                          int total_lines);
