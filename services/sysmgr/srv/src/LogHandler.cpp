...
int LogHandler::identify_chain_element(void *element, int ident,
...
int LogHandler::free_chain_element_data(void *element, ADChainProducer *pObj) {
  // nothing to free here, coz LOG_MSG_HOLDER doesnt have any dynamically
  // allocated memory pointer
  return 0;
}
/* ------------------------------------------------------------------------- */
int LogHandler::grep_log_message_line(char *log_filepath, char *match_string,
                                      int line, char *message) {
  char command[1024];
  char temp_str[MAX_LOG_MESSAGE_LENGTH];
  // grep SDSRV file | sed '3,3!d' (prints the 3rd line)
  sprintf(command, "grep %s %s | sed '%d,%d!d' ", match_string, log_filepath,
          line, line); // grep the line-range
  FILE *shell;
...
/* ------------------------------------------------------------------------- */
int LogHandler::push_log_message(int line, char *message,
                                 ADGenericChain *pChain) {
  ...
  if (pChain->chain_put((void *)pHolder) != 0) {
    ...
    // LOG_ERR_MSG("SDSRV:sdsms","SdsmsLog::push_log_message:unable to push item
    // in chain");
    return -1;
  }
  return 0;
...
/* ------------------------------------------------------------------------- */
int LogHandler::update_logmsg_chain(char *log_filepath, char *match_string,
                                    int total_lines) {
...
/* ------------------------------------------------------------------------- */
RPC_SRV_RESULT LogHandler::read_log_message_count(char *log_filepath,
                                                  char *match_string,
                                                  int *TotalMessages) {
  int count = 0;
  char command[1024];
  char temp_str[256];
  sprintf(command, "grep %s %s | sed -n '$='", match_string,
          log_filepath); // count number of lines
  FILE *shell;
  shell = popen(command, "r");
  if (shell == NULL)
...
RPC_SRV_RESULT LogHandler::UpdateLogList() {
  ...
  read_log_message_count((char *)LogFilePath.c_str(),
                         (char *)LOG_MSG_MATCH_STRING, &TotalMessages);
  return RPC_SRV_RESULT_SUCCESS;
}
/* ------------------------------------------------------------------------- */
int LogHandler::GetLogLines() { return logmsg_chain.get_chain_size(); }
/* ------------------------------------------------------------------------- */
// before calling this function, make sure that chain is updated by calling
// UpdateLogList
RPC_SRV_RESULT LogHandler::read_log_message_line(int line_num, char *message) {
  ...
  pHolder =
      (LOG_MSG_PACKET *)logmsg_chain.chain_get_by_ident(temp); // line_num);
