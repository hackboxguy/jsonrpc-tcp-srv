...
/* ------------------------------------------------------------------------- */
EventMonitor::EventMonitor(std::string rpcName, int myIndex, bool emu, bool log,
                           SYSMGR_CMN_DATA_CACHE *pData)
    : ADJsonRpcMgrConsumer(rpcName, myIndex, emu, log) {
  pDataCache = pData;
  if (pDataCache->EvntMonitorConfigFile ==
      "") // nothing to do here if config file is empty
    return;
...
/* ------------------------------------------------------------------------- */
void EventMonitor::ReceiveEvent(int myToken, int evntNum, int evntArg,
                                int evntArg2) {
  if (pDataCache->EvntMonitorConfigFile ==
      "") // nothing to do here if config file is empty
    return;
  ...
  std::vector<EvntMonitEntry>::iterator it = find_if(
      EventMonitList.begin(), EventMonitList.end(), FindMonitEntry(myToken));
  if (it != EventMonitList.end()) {
    if (evntNum == 1) // 1 is for shutdown, indicates that server is going
                      // bye-bye, hence disable this entry from my list.
    ...
    if (evntNum <= 11) // max 11 event types are allowed
      cout << get_timestamp(mytimestamp) << " | ip=" << (*it).ip
           << " | token=" << (*it).srvToken << " | evntType=" << setw(25)
           << left << (*it).EvntName[evntNum] << " | Arg1=" << myarg1
           << " | Arg2=" << myarg2 << endl;
  }
}
/* ------------------------------------------------------------------------- */
RPC_SRV_RESULT
EventMonitor::LoadEventMonitList(std::string listFile,
                                 std::vector<EvntMonitEntry> *pList) {
  if (listFile.size() <= 0)
  ...
  while (std::getline(infile, line)) {
    ...
    pList->push_back(EvntMonitEntry(strip, port, srvToken, evntType, str1, str2,
                                    str3, str4, str5, str6, str7));
    // cout<<"ip="<<strip<<" port="<<port<<" srvToken="<<srvToken<<"
    // evntType="<<evntType<<" str1="<<str1<<" str2="<<str2<<" str3="<<str3<<"
    // str4="<<str4<<" str5="<<str5<<" str6="<<str7<<" str7="<<str6<<endl;
  }
  return RPC_SRV_RESULT_SUCCESS;
}
/* ------------------------------------------------------------------------- */
RPC_SRV_RESULT
EventMonitor::SubscribeEvents(std::vector<EvntMonitEntry> *pList) {
  std::vector<EvntMonitEntry>::iterator it;
  for (it = pList->begin(); it != pList->end(); it++) {
    EvntMonitEntry pEntry = (*it);
    if (pEntry.subscription_sts == false) {
      pEntry.subscription = -1;
      SUBSCRIBE_EVENT(pEntry.ip, pEntry.port, &pEntry.subscription,
                      pEntry.srvToken, pEntry.evntType, 40001);
      if (pEntry.subscription != -1)
        pEntry.subscription_sts = true;
      else {
        char mytimestamp[255];
        cout << get_timestamp(mytimestamp)
             << " | subscription failed for ip=" << pEntry.ip
             << " and token=" << pEntry.srvToken << endl;
...
/* ------------------------------------------------------------------------- */
RPC_SRV_RESULT
EventMonitor::UnSubscribeEvents(std::vector<EvntMonitEntry> *pList) {
  std::vector<EvntMonitEntry>::iterator it;
  for (it = pList->begin(); it != pList->end(); it++) {
    EvntMonitEntry pEntry = (*it);
