...
DevIdentGlMt300nv2::DevIdentGlMt300nv2() {
  ...
  // char command[255];
  // sprintf(command,"echo 0 >
  // /sys/class/leds/gl-mt300n-v2\\:green\\:power/brightness"); system(command);
  // sprintf(command,"echo 0 >
  // /sys/class/leds/gl-mt300n-v2\\:green\\:wan/brightness"); system(command);
}
/* ------------------------------------------------------------------------- */
DevIdentGlMt300nv2::~DevIdentGlMt300nv2() {}
/* ------------------------------------------------------------------------- */
RPC_SRV_RESULT DevIdentGlMt300nv2::device_identify() {
  ...
  // preserve last state
  sprintf(command, "cat /sys/class/leds/green:power/brightness > "
                   "/tmp/last-power-led-state.txt");
  system(command);

  for (int i = 0; i < 10; i++) {
    // sprintf(command,"echo 1 >
  ...
  sprintf(command, "echo $(cat /tmp/last-power-led-state.txt) > "
