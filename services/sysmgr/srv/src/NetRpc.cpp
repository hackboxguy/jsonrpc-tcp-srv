...
/* ------------------------------------------------------------------------- */
NetRpc::NetRpc(std::string rpcName, int myIndex, bool emu, bool log,
...
int NetRpc::MapJsonToBinary(JsonDataCommObj *pReq, int index) {
  ...
  case EJSON_SYSMGR_RPC_SET_MAC_ADDR:
    return json_to_bin_set_mac_addr(pReq);
  case EJSON_SYSMGR_RPC_GET_ETH_COUNT:
    return json_to_bin_get_eth_count(pReq);
  case EJSON_SYSMGR_RPC_GET_ETH_NAME:
    return json_to_bin_get_eth_name(pReq);
  case EJSON_SYSMGR_RPC_GET_IP_ADDR:
    return json_to_bin_get_ip_addr(pReq);
  case EJSON_SYSMGR_RPC_SET_IP_ADDR:
    return json_to_bin_set_ip_addr(pReq);
...
int NetRpc::MapBinaryToJson(JsonDataCommObj *pReq, int index) {
  ...
  case EJSON_SYSMGR_RPC_SET_MAC_ADDR:
    return bin_to_json_set_mac_addr(pReq);
  case EJSON_SYSMGR_RPC_GET_ETH_COUNT:
    return bin_to_json_get_eth_count(pReq);
  case EJSON_SYSMGR_RPC_GET_ETH_NAME:
    return bin_to_json_get_eth_name(pReq);
  case EJSON_SYSMGR_RPC_GET_IP_ADDR:
    return bin_to_json_get_ip_addr(pReq);
  case EJSON_SYSMGR_RPC_SET_IP_ADDR:
    return bin_to_json_set_ip_addr(pReq);
  case EJSON_SYSMGR_RPC_GET_NETMASK:
...
int NetRpc::ProcessWork(JsonDataCommObj *pReq, int index,
  ...
  case EJSON_SYSMGR_RPC_SET_MAC_ADDR:
    return process_set_mac_addr(pReq);
  case EJSON_SYSMGR_RPC_GET_ETH_COUNT:
    return process_get_eth_count(pReq);
  case EJSON_SYSMGR_RPC_GET_ETH_NAME:
    return process_get_eth_name(pReq);
  case EJSON_SYSMGR_RPC_GET_IP_ADDR:
    return process_get_ip_addr(pReq);
  case EJSON_SYSMGR_RPC_SET_IP_ADDR:
    return process_set_ip_addr(pReq);
  case EJSON_SYSMGR_RPC_GET_NETMASK:
...
int NetRpc::json_to_bin_get_mac_addr(JsonDataCommObj *pReq) {
  ...
  PREPARE_JSON_REQUEST(RPC_SRV_REQ, SYSMGR_MAC_ADDR_PACKET, RPC_SRV_ACT_READ,
  ...
  JSON_STRING_TO_STRING(
      ...
      pPanelCmdObj->eth_name); // changed from fixed enum list to string
  return 0;
}
int NetRpc::bin_to_json_get_mac_addr(JsonDataCommObj *pReq) {
  PREPARE_JSON_RESP_STRING(RPC_SRV_REQ, SYSMGR_MAC_ADDR_PACKET,
                           SYSMGR_RPC_MAC_ADDR_ARG, mac_addr);
  return 0;
}
int NetRpc::process_get_mac_addr(JsonDataCommObj *pReq) {
  ...
  if (SysInfo.read_network_info(pPacket->eth_name, pPacket->mac_addr, ip,
    ...
    if (SysInfo.read_network_info_ifconfig(pPacket->eth_name, pPacket->mac_addr,
                                           ip, netmask) == 0)
      pPanelReq->result = RPC_SRV_RESULT_SUCCESS;
    else
...
int NetRpc::json_to_bin_set_mac_addr(JsonDataCommObj *pReq) {
  SYSMGR_MAC_ADDR_PACKET *pPanelCmdObj = NULL;
  PREPARE_JSON_REQUEST(RPC_SRV_REQ, SYSMGR_MAC_ADDR_PACKET, RPC_SRV_ACT_WRITE,
                       EJSON_SYSMGR_RPC_SET_MAC_ADDR);
  // after assigning pointer to pPanelReq->dataRef, modify cmd-req-obj with
  // correct client arguments
  JSON_STRING_TO_ENUM(SYSMGR_RPC_MAC_ADDR_ARG_IFACE,
                      SYSMGR_RPC_MAC_ADDR_ARG_IFACE_TABL,
                      EJSON_SYSMGR_IFACE_TYPE, EJSON_SYSMGR_IFACE_TYPE_UNKNOWN,
                      pPanelCmdObj->eth_type);
  return 0;
}
...
int NetRpc::json_to_bin_get_eth_count(JsonDataCommObj *pReq) {
  ...
  PREPARE_JSON_REQUEST(RPC_SRV_REQ, SYSMGR_ETH_COUNT_PACKET, RPC_SRV_ACT_READ,
                       EJSON_SYSMGR_RPC_GET_ETH_COUNT);
  return 0;
}
int NetRpc::bin_to_json_get_eth_count(JsonDataCommObj *pReq) {
  PREPARE_JSON_RESP_INT(RPC_SRV_REQ, SYSMGR_MAC_ADDR_PACKET,
                        SYSMGR_RPC_ETH_COUNT_ARG, EthCountIndx);
  return 0;
}
...
int NetRpc::json_to_bin_get_eth_name(JsonDataCommObj *pReq) {
  ...
  PREPARE_JSON_REQUEST(RPC_SRV_REQ, SYSMGR_ETH_NAME_PACKET, RPC_SRV_ACT_READ,
...
int NetRpc::bin_to_json_get_eth_name(JsonDataCommObj *pReq) {
  PREPARE_JSON_RESP_STRING(RPC_SRV_REQ, SYSMGR_ETH_NAME_PACKET,
                           SYSMGR_RPC_ETH_NAME_ARG, eth_name);
  return 0;
}
int NetRpc::process_get_eth_name(JsonDataCommObj *pReq) {
  ...
  pPanelReq->result =
      SysInfo.get_nth_eth_name(pPacket->EthCountIndx + 1, pPacket->eth_name);
  return 0;
}
/* ------------------------------------------------------------------------- */
int NetRpc::json_to_bin_get_ip_addr(JsonDataCommObj *pReq) {
  SYSMGR_NET_INFO_PACKET *pPanelCmdObj = NULL;
  PREPARE_JSON_REQUEST(RPC_SRV_REQ, SYSMGR_NET_INFO_PACKET, RPC_SRV_ACT_READ,
...
int NetRpc::bin_to_json_get_ip_addr(JsonDataCommObj *pReq) {
  PREPARE_JSON_RESP_STRING(RPC_SRV_REQ, SYSMGR_NET_INFO_PACKET,
                           SYSMGR_RPC_IP_ADDR_ARG, addr);
  return 0;
}
int NetRpc::process_get_ip_addr(JsonDataCommObj *pReq) {
  ...
  if (SysInfo.read_network_info(pPacket->eth_name, mac, pPacket->addr,
    ...
    if (SysInfo.read_network_info_ifconfig(pPacket->eth_name, mac,
                                           pPacket->addr, netmask) == 0)
      pPanelReq->result = RPC_SRV_RESULT_SUCCESS;
    else
      pPanelReq->result = RPC_SRV_RESULT_FAIL;
  }
...
int NetRpc::json_to_bin_set_ip_addr(JsonDataCommObj *pReq) {
  SYSMGR_NET_INFO_PACKET *pPanelCmdObj = NULL;
  PREPARE_JSON_REQUEST(RPC_SRV_REQ, SYSMGR_NET_INFO_PACKET, RPC_SRV_ACT_WRITE,
                       EJSON_SYSMGR_RPC_SET_IP_ADDR);
  // after assigning pointer to pPanelReq->dataRef, modify cmd-req-obj with
  // correct client arguments
  JSON_STRING_TO_ENUM(SYSMGR_RPC_IP_ADDR_ARG_IFACE,
...
int NetRpc::process_get_netmask(JsonDataCommObj *pReq) {
  ...
  if (SysInfo.read_network_info(pPacket->eth_name, mac, ip, pPacket->addr) == 0)
    ...
    if (SysInfo.read_network_info_ifconfig(pPacket->eth_name, mac, ip,
                                           pPacket->addr) == 0)
      pPanelReq->result = RPC_SRV_RESULT_SUCCESS;
    else
      pPanelReq->result = RPC_SRV_RESULT_FAIL;
...
int NetRpc::json_to_bin_set_netmask(JsonDataCommObj *pReq) {
  SYSMGR_NET_INFO_PACKET *pPanelCmdObj = NULL;
  PREPARE_JSON_REQUEST(RPC_SRV_REQ, SYSMGR_NET_INFO_PACKET, RPC_SRV_ACT_WRITE,
                       EJSON_SYSMGR_RPC_SET_NETMASK);
  // after assigning pointer to pPanelReq->dataRef, modify cmd-req-obj with
  // correct client arguments
  JSON_STRING_TO_ENUM(SYSMGR_RPC_NETMASK_ARG_IFACE,
                      SYSMGR_RPC_NETMASK_ARG_IFACE_TABL,
                      EJSON_SYSMGR_IFACE_TYPE, EJSON_SYSMGR_IFACE_TYPE_UNKNOWN,
                      pPanelCmdObj->eth_type);
  return 0;
}
