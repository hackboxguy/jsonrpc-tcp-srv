...
MyCmdline::MyCmdline(CMDLINE_HELPER_MODE cmdline_mode, int portnum,
  ...
  CmdlineHelper.insert_options_entry((char *)"evntmonitcfg", optional_argument,
                                     SYSMGR_CMDLINE_OPT_MONITCFG);
  CmdlineHelper.insert_help_entry(
      (char *)"--evntmonitcfg=filepath         (config file for monitoring the "
              "event)");
  ...
  CmdlineHelper.insert_options_entry((char *)"disptype", optional_argument,
                                     SYSMGR_CMDLINE_OPT_DISPTYPE);
  CmdlineHelper.insert_help_entry(
      (char *)"--disptype=type                 (display-type "
              "[ssd1306_128x32/ssd1306_128x64/1602_pcf/1604_pcf/2002_pcf/"
              "2004_pcf])");
  CmdlineHelper.insert_options_entry((char *)"device", optional_argument,
                                     SYSMGR_CMDLINE_OPT_DISPDEVICE);
  CmdlineHelper.insert_help_entry(
      (char *)"--device=/dev/i2c-x         (i2c device node if applicable)");
}
/*****************************************************************************/
MyCmdline::~MyCmdline() {}
/*****************************************************************************/
...
int MyCmdline::parse_my_cmdline_options(int arg, char *sub_arg) {
  ...
  case SYSMGR_CMDLINE_OPT_MONITCFG:
    if (CmdlineHelper.get_next_subargument(&sub_arg) ==
        0) // no /dev/tty option passed by user
      MonitCfgFile = "";
    else
  ...
  case SYSMGR_CMDLINE_OPT_DISPDEVICE:
    if (CmdlineHelper.get_next_subargument(&sub_arg) ==
        0)                    // no /dev/i2c-x option passed by user
...
/*****************************************************************************/
std::string MyCmdline::get_monit_cfg_file() { return MonitCfgFile; }
/*****************************************************************************/
...
/*****************************************************************************/
std::string MyCmdline::get_disp_type() { return DisplayType; }
/*****************************************************************************/
std::string MyCmdline::get_dev_node() { return DevNode; }
/*****************************************************************************/
