...
typedef enum EJSON_SYSMGR_RPC_TYPES_T {
  ...
  EJSON_SYSMGR_RPC_GET_ETH_COUNT, // returns number of eth interfaces available
                                  // on the device
  ...
  EJSON_SYSMGR_RPC_GET_BOOT_SYSTEM, // returns current booted partition brbox1
                                    // or brbox2
  ...
  EJSON_SYSMGR_RPC_SET_UPDATE_LOG,       // reads BRBOX specific log lines from
                                   // /tmp/messages and fills in vectorList
  EJSON_SYSMGR_RPC_GET_LOG_COUNT, // returns total number of log lines which was
                                  // filled in vectorList
  EJSON_SYSMGR_RPC_GET_LOG_LINE,  // returns log-message of a given index from
                                  // vectorList
  ...
  EJSON_SYSMGR_RPC_EVNT_SUBSCRIBE,   // custom event handler to re-subscribe
                                     // events as per given event-config
  ...
  EJSON_SYSMGR_RPC_RUN_SHELLCMDTRIG, // runs a shell command but doesnt
                                     // re-direct to /tmp/shellcmd.resp
...
#define SYSMGR_RPC_MAC_ADDR_ARG_IFACE_TABL                                     \
  {                                                                            \
    "eth0", "eth1", "wlan0", "br-lan", "eth0.1", "eth0.2", "br0", "tun1",      \
        "enp110s0", "usb0", "unknown", "none", "\0"                            \
  }
...
typedef struct SYSMGR_MAC_ADDR_PACKET_T // common communication object for
                                        // different mac related rpc's
{
...
#define SYSMGR_RPC_DEV_OP_ARG_TABL                                             \
  {                                                                            \
    "idle", "on", "laststate", "standby", "reboot", "booting", "rebooting",    \
        "switchingoff", "boot", "unknown", "none", "\0"                        \
  }
...
typedef struct SYSMGR_DEV_OP_PACKET_T {
  ...
  char operation_str[255]; // needed for settings store/restore(un-used for
                           // json-rpc mappers)
...
#define SYSMGR_RPC_FMW_VER_ARG_TABL                                            \
  {                                                                            \
    "current", "backup", "kernel", "project", "brbox1", "brbox2", "unknown",   \
        "none", "\0"                                                           \
  }
...
typedef struct SYSMGR_FMW_MODULE_PACKET_T {
  ...
  char cmn_fname_ver_str[512]; // common for exchanging fmw_file_path or version
                               // string(MAX_FILE_PATH is 512bytes)
  int taskID; // for fmw update, return the taskID so that client can check the
              // progress
...
#define SYSMGR_RPC_BOOT_SYSTEM_ARG_TABL                                        \
  { "brbox1", "brbox2", "unknown", "none", "\0" }
...
#define SYSMGR_RPC_DOWNLOADFILE_ARG_SRCADDR                                    \
  "srcaddr" // incase of ftp it is the url, incase of tftp it is the serverip
...
typedef struct SYSMGR_DOWNLOAD_FILE_PACKET_T {
  ...
  char sourcefilepath[1024]; // filepath of on the source file incase of tftp
                             // download
  char targetfilepath[1024]; // filepath of on the target where file to be
                             // downloaded
  int taskID; // for file download, return the taskID so that client can check
              // the progress
...
#define SYSMGR_RPC_ASYNCTASK_ARG_TABL                                          \
  {                                                                            \
    "devop", "fupdate", "ftpdownload", "tftpdownload", "loglistupdt",          \
        "shellcmd", "devident", "shellcmdtrig", "none", "none", "\0"           \
...
typedef struct SYSMGR_NET_INFO_PACKET_T // common communication object for
                                        // netinfo related rpc's
{
  EJSON_SYSMGR_IFACE_TYPE eth_type;
  char addr[512];
  char eth_name[512];
...
#define SYSMGR_RPC_DISP_LINE_ARG_TABL                                          \
  { "line1", "line2", "line3", "line4", "lineall", "none", "none", "\0" }
...
#define SYSMGR_RPC_DISP_BKLT_ARG_TABL                                          \
  { "off", "on", "unknown", "none", "\0" }
