...
XMPROXY_CMD_TABLE xmproxy_cmd_table[] = // EXMPP_CMD_NONE+1] =
    {
        ...
        {true, EXMPP_CMD_SHUTDOWN, "xmpshutdown", "",
         EXMPP_USER_ACCESS_ADMIN}, // shutdown xmpp server(for xmpp logout)
        {true, EXMPP_CMD_SONOFF, "sonoff", "<ip/hostname> [sts(on/off/toggle)]",
         EXMPP_USER_ACCESS_READWRITE}, // http based control of sonoff
                                       // relay(tasmota fmw)
        {true, EXMPP_CMD_DISPCLEAR, "dispclear", "",
         EXMPP_USER_ACCESS_READWRITE},
        {true, EXMPP_CMD_DISPPRINT, "display",
         "<line1/line2/line3..]> <message>", EXMPP_USER_ACCESS_READWRITE},
        {true, EXMPP_CMD_DISPBKLT, "dispbklt", "<sts[on/off]>",
         EXMPP_USER_ACCESS_READWRITE},
        {true, EXMPP_CMD_BUDDY_ADD, "acceptbuddy", "[buddyID]",
         EXMPP_USER_ACCESS_ADMIN}, // only admin-buddy can run this command
        {true, EXMPP_CMD_BUDDY_REMOVE, "rejectbuddy", "[buddyID]",
         EXMPP_USER_ACCESS_ADMIN}, // only admin-buddy can run this command
        {true, EXMPP_CMD_BUDDY_SUBSCRIBE, "subscribe", "[buddyID]",
         EXMPP_USER_ACCESS_ADMIN}, // only admin-buddy can run this command
...
void XmppMgr::SetAiAgentUrl(std::string url) {
  ...
  botcli->set_connection_timeout(
      MAX_INFERENCE_TIME, 0); // time taken to serve the query from the ai model
  botcli->set_read_timeout(
      MAX_INFERENCE_TIME, 0); // time taken to serve the query from the ai model
  botcli->set_write_timeout(
      MAX_INFERENCE_TIME, 0); // time taken to serve the query from the ai model
#endif
}
/* ------------------------------------------------------------------------- */
...
/* ------------------------------------------------------------------------- */
#ifdef USE_CXMPP_LIB
int XmppMgr::onXmppMessage(std::string msg, std::string sender,
                           CXmppProducer *pObj)
#else
int XmppMgr::onXmppMessage(std::string msg, std::string sender,
                           ADXmppProducer *pObj)
...
RPC_SRV_RESULT XmppMgr::AccessAsyncTaskList(int tid, int port,
  ...
  if (insertEntryFlag == true) {
    ...
    std::vector<AyncEventEntry>::iterator it =
        find_if(AsyncTaskList.begin(), AsyncTaskList.end(),
                FindAsyncEventEntry(tid, port));
    ...
    AsyncTaskList.erase(remove_if(AsyncTaskList.begin(), AsyncTaskList.end(),
                                  FindAsyncEventEntry(tid, port)),
                        AsyncTaskList.end());
...
EXMPP_CMD_TYPES XmppMgr::ResolveCmdStr(std::string msg) {
  ...
  //--------------------------------------------------------------------
  xmpcmd =
  (EXMPP_CMD_TYPES)string_proc.string_to_enum(cmdTbl,(char*)cmd.c_str(),EXMPP_CMD_UNKNOWN);
  ...
  for (int i = 0; i < total_cmds; i++) {
    if (xmproxy_cmd_table[i].cmdsts != true)
      continue; // dont support disabled commands
...
RPC_SRV_RESULT XmppMgr::set_online_status(bool status) {
  if (status == true) // go online
    XmppProxy.setOnDemandDisconnect(
        false); // thread will notice this in next 5sec sampling, and then will
                // go online
  else          // go offline
  {
...
RPC_SRV_RESULT XmppMgr::SendMessage(std::string msg) {
  ...
  // RPC_SRV_RESULT
  // res=GetOrUpdateTotal(SMS_ACCESS_READ_INDEX,tot,index,message,frm,dummyList);
...
RPC_SRV_RESULT XmppMgr::proc_cmd_sms_deleteall(std::string msg,
  ...
  if (Client.rpc_server_connect(bboxSmsServerAddr.c_str(),
                                ADCMN_PORT_BBOXSMS) != 0)
    return RPC_SRV_RESULT_HOST_NOT_REACHABLE_ERR;
  RPC_SRV_RESULT result = Client.set_action_noarg_get_single_string_type(
      (char *)"delete_all_sms", (char *)"taskId", tID);
...
RPC_SRV_RESULT XmppMgr::proc_cmd_sms_get(std::string msg,
  ...
  char returnmsg[1024];
  returnmsg[0] = '\0';
  RPC_SRV_RESULT result = Client.get_int_type_with_string_para(
      (char *)"get_sms", (char *)"index", msgIndex, returnmsg,
      (char *)"message");
  returnval = returnmsg;
  Client.rpc_server_disconnect();
  return result;
...
RPC_SRV_RESULT XmppMgr::proc_cmd_sms_send(std::string msg,
  ...
  if (Client.rpc_server_connect(bboxSmsServerAddr.c_str(),
                                ADCMN_PORT_BBOXSMS) != 0)
    return RPC_SRV_RESULT_HOST_NOT_REACHABLE_ERR;
  RPC_SRV_RESULT result = Client.set_double_string_get_single_string_type(
      (char *)"send_sms", (char *)"destination", (char *)destArg.c_str(),
      (char *)"message", (char *)msgArg.c_str(), (char *)"taskId", (char *)tID);
  Client.rpc_server_disconnect();
  returnval = "taskID="; // returnval+=tID;
  int xmptid = -1;
  if (result == RPC_SRV_RESULT_IN_PROG)
...
RPC_SRV_RESULT XmppMgr::proc_cmd_sms_list_update(std::string msg,
  ...
  if (Client.rpc_server_connect(bboxSmsServerAddr.c_str(),
    ...
    return RPC_SRV_RESULT_HOST_NOT_REACHABLE_ERR;
  RPC_SRV_RESULT result = Client.set_action_noarg_get_single_string_type(
      (char *)"update_sms_list", (char *)"taskId", tID);
  Client.rpc_server_disconnect();
  returnval = "taskID="; // returnval+=tID;
  int xmptid = -1;
  if (result == RPC_SRV_RESULT_IN_PROG)
...
RPC_SRV_RESULT XmppMgr::proc_cmd_sms_get_total(std::string msg,
  ...
  if (Client.rpc_server_connect(bboxSmsServerAddr.c_str(),
                                ADCMN_PORT_BBOXSMS) != 0)
    return RPC_SRV_RESULT_HOST_NOT_REACHABLE_ERR;
  RPC_SRV_RESULT result = Client.get_integer_type((char *)"get_total_sms",
                                                  (char *)"msgcount", temp_str);
  Client.rpc_server_disconnect();
  returnval = temp_str;
  return result;
}
...
RPC_SRV_RESULT XmppMgr::proc_cmd_fmw_get_version(std::string msg,
                                                 std::string &returnval) {
  // 01:39:17.322-->{ "jsonrpc": "2.0", "method": "get_fmw_version", "params": {
  // "module": "current" }, "id": 0 } 01:39:17.408<--{ "jsonrpc": "2.0",
  // "result": { "return": "Success", "version": "00.01.00341" }, "id": 0 }
  char temp_str[255];
  temp_str[0] = '\0';
  ADJsonRpcClient Client;
  ...
  RPC_SRV_RESULT result = Client.get_string_type_with_string_para(
      (char *)"get_fmw_version", (char *)"module", (char *)"current", temp_str,
      (char *)"version");
  Client.rpc_server_disconnect();
  returnval = temp_str;
  return result;
...
RPC_SRV_RESULT XmppMgr::proc_cmd_fmw_update(std::string msg,
                                            std::string &returnval,
                                            std::string sender) {
  // 00:06:05.434-->{ "jsonrpc": "2.0", "method": "download_ftp_file", "params":
  // { "srcaddr":
  // "http:\/\/github.com\/hackboxguy\/downloads\/raw\/master\/bbbmmcRbox\/readme.txt",
  // "targetpath": "\/tmp\/readme.txt" }, "id": 0 } 00:06:05.454<--{ "jsonrpc":
  // "2.0", "result": { "return": "InProgress", "taskId": 1 }, "id": 0 }
  // github-file-download-cmd: wget -O /tmp/uBrBox.uimg
  // http://github.com/hackboxguy/downloads/raw/master/README.md
  ...
  if (UpdateUrlFile != "") {
    ...
    if (customurl.size() <= 0) {
      URLPath = GITHUB_FMW_DOWNLOAD_FOLDER;
      URLPath += line;
    } else {
      URLPath = customurl;
      LOG_DEBUG_MSG_1_ARG(DebugLog, "BRBOX:xmproxy",
                          "XmppMgr::proc_cmd_fmw_update::using customURL=%s",
                          customurl.c_str());
    }
  } else {
    URLPath = GITHUB_FMW_DOWNLOAD_FOLDER;
  ...
  RPC_SRV_RESULT result = Client.set_double_string_get_single_string_type(
      (char *)"download_ftp_file", (char *)"srcaddr", (char *)URLPath.c_str(),
      (char *)"targetpath", (char *)"/tmp/uBrBoxRoot.uimg", (char *)"taskId",
      temp_str);
  // Note: for caller, its one cmd, but internally there are two(download &
  // update) pushAsyncEvent ID only for update-cmd(ignore download-complete
  // taskEvent).

  // fmw update rpc call
  // 01:19:27.440-->{ "jsonrpc": "2.0", "method": "firmware_update", "params": {
  // "module": "project", "filepath": "\/tmp\/messages" }, "id": 0 }
  // 01:19:27.449<--{ "jsonrpc": "2.0", "result": { "return": "InProgress",
  // "taskId": 4 }, "id": 0 }
  if (result == RPC_SRV_RESULT_IN_PROG) {
    result = Client.set_double_string_get_single_string_type(
        (char *)"firmware_update", (char *)"module", (char *)"project",
        (char *)"filepath", (char *)"/tmp/uBrBoxRoot.uimg", (char *)"taskId",
        temp_str);
...
RPC_SRV_RESULT XmppMgr::proc_cmd_fmw_reboot(std::string msg,
                                            std::string &returnval,
                                            std::string sender) {
  // 01:39:52.718-->{ "jsonrpc": "2.0", "method": "set_device_operation",
  // "params": { "operation": "reboot" }, "id": 0 } 01:39:52.726<--{ "jsonrpc":
  // "2.0", "result": { "return": "InProgress", "taskId": 1 }, "id": 0 }
  char temp_str[255];
  temp_str[0] = '\0';
  ADJsonRpcClient Client;
  ...
  RPC_SRV_RESULT result = Client.set_single_string_get_single_string_type(
      (char *)"set_device_operation", (char *)"operation", (char *)"reboot",
...
RPC_SRV_RESULT XmppMgr::proc_cmd_fmw_poweroff(std::string msg,
                                              std::string &returnval,
                                              std::string sender) {
  char temp_str[255];
  temp_str[0] = '\0';
  ...
  if (Client.rpc_server_connect(bboxSmsServerAddr.c_str(), ADCMN_PORT_SYSMGR) !=
    ...
    return RPC_SRV_RESULT_HOST_NOT_REACHABLE_ERR;
  RPC_SRV_RESULT result = Client.set_single_string_get_single_string_type(
      (char *)"set_device_operation", (char *)"operation", (char *)"standby",
      (char *)"taskId", temp_str);
  Client.rpc_server_disconnect();
  returnval = "taskID="; // returnval+=temp_str;
  int xmptid = -1;
...
RPC_SRV_RESULT XmppMgr::proc_cmd_fmw_update_sts(std::string msg,
  ...
  // 00:40:37.480<--{ "jsonrpc": "2.0", "result": { "return": "Success", "task":
  // "none" }, "id": 0 }
  char temp_str[255];
  temp_str[0] = '\0';
  ADJsonRpcClient Client;
  if (Client.rpc_server_connect(bboxSmsServerAddr.c_str(), ADCMN_PORT_SYSMGR) !=
      0)
    return RPC_SRV_RESULT_HOST_NOT_REACHABLE_ERR;
  RPC_SRV_RESULT result = Client.get_string_type((char *)"get_async_task",
                                                 (char *)"task", temp_str);
  Client.rpc_server_disconnect();
  returnval = temp_str;
...
RPC_SRV_RESULT XmppMgr::proc_cmd_fmw_update_res(std::string msg,
  ...
  // "params": { "taskId": "1" }, "id": 0 } 01:02:29.615<--{ "jsonrpc": "2.0",
  // "result": { "return": "Success", "taskStatus": "taskIDNotFound" }, "id": 0
  // }
  char tRes[255];
  tRes[0] = '\0';
  ADJsonRpcClient Client;
...
RPC_SRV_RESULT XmppMgr::proc_cmd_fmw_uptime(std::string msg,
  ...
  // 01:42:50.078<--{ "jsonrpc": "2.0", "result": { "return": "Success",
  // "current": "0", "average": "0", "uptime": "2632" }, "id": 0 }
  char str_cur[255];
  str_cur[0] = '\0';
  char str_avg[255];
  str_avg[0] = '\0';
  ...
  if (Client.rpc_server_connect(bboxSmsServerAddr.c_str(), ADCMN_PORT_SYSMGR) !=
      0)
    return RPC_SRV_RESULT_HOST_NOT_REACHABLE_ERR;
  RPC_SRV_RESULT result = Client.get_three_string_type(
      (char *)"get_load_info", (char *)"current", str_cur, (char *)"average",
      str_avg, (char *)"uptime", str_uptm);
  Client.rpc_server_disconnect();
...
/* ------------------------------------------------------------------------- */
RPC_SRV_RESULT XmppMgr::proc_cmd_fmw_get_hostname(std::string msg,
                                                  std::string &returnval) {
  // 14:52:09.440-->{ "jsonrpc": "2.0", "method": "get_debug_outfile", "id": 0 }
  // 14:52:09.440<--{ "jsonrpc": "2.0", "result": { "return": "Success",
  // "filepath": "" }, "id": 0 }
  char temp_str[255];
  temp_str[0] = '\0';
  ADJsonRpcClient Client;
  if (Client.rpc_server_connect(bboxSmsServerAddr.c_str(), ADCMN_PORT_SYSMGR) !=
      0)
    return RPC_SRV_RESULT_HOST_NOT_REACHABLE_ERR;
  RPC_SRV_RESULT result = Client.get_string_type((char *)"get_hostname",
...
RPC_SRV_RESULT XmppMgr::proc_cmd_fmw_set_hostname(std::string msg) {
  // 14:52:21.336-->{ "jsonrpc": "2.0", "method": "set_debug_outfile", "params":
  // { "filepath": "\/home\/adav\/tmp" }, "id": 0 } 14:52:21.336<--{ "jsonrpc":
  // "2.0", "result": { "return": "Success" }, "id": 0 }
  std::string cmd, cmdArg;
  stringstream msgstream(msg);
  msgstream >> cmd;
  msgstream >> cmdArg;
  ...
  if (Client.rpc_server_connect(bboxSmsServerAddr.c_str(), ADCMN_PORT_SYSMGR) !=
      0)
    return RPC_SRV_RESULT_HOST_NOT_REACHABLE_ERR;
  RPC_SRV_RESULT result = Client.set_single_string_type(
      (char *)"set_hostname", (char *)"hostname", (char *)cmdArg.c_str());
  Client.rpc_server_disconnect();
  return result;
}
/* ------------------------------------------------------------------------- */
RPC_SRV_RESULT XmppMgr::proc_cmd_fmw_hostname(std::string msg,
                                              std::string &returnval) {
  std::string cmd, cmdArg;
  stringstream msgstream(msg);
  msgstream >> cmd;
...
RPC_SRV_RESULT XmppMgr::proc_cmd_gpio(std::string msg, std::string &returnval) {
  ...
  if (cmdArg2.size() <= 0) // read gpio pin
  {
    ...
    ADJsonRpcClient Client;
    if (Client.rpc_server_connect(bboxSmsServerAddr.c_str(),
                                  ADCMN_PORT_GPIOCTL) != 0)
      return RPC_SRV_RESULT_HOST_NOT_REACHABLE_ERR;
    RPC_SRV_RESULT result = Client.get_integer_type_with_addr_para(
        (char *)"gpio_get", (char *)"addr", gpioAddr, (char *)"data", temp_str);
    Client.rpc_server_disconnect();
    returnval = temp_str;
    return result;
  } else // write gpio pin
  {
    ...
    if (Client.rpc_server_connect(bboxSmsServerAddr.c_str(),
                                  ADCMN_PORT_GPIOCTL) != 0)
      return RPC_SRV_RESULT_HOST_NOT_REACHABLE_ERR;
    RPC_SRV_RESULT result = Client.set_integer_type_with_addr_para(
        (char *)"gpio_set", (char *)"addr", gpioAddr, (char *)"data", gpioVal);
    Client.rpc_server_disconnect();
    return result;
  }
}
/* ------------------------------------------------------------------------- */
RPC_SRV_RESULT XmppMgr::proc_cmd_fmw_get_myip(std::string msg,
                                              std::string &returnval) {
  // 14:52:09.440-->{ "jsonrpc": "2.0", "method": "get_debug_outfile", "id": 0 }
  // 14:52:09.440<--{ "jsonrpc": "2.0", "result": { "return": "Success",
  // "filepath": "" }, "id": 0 }
  ...
  if (Client.rpc_server_connect(bboxSmsServerAddr.c_str(), ADCMN_PORT_SYSMGR) !=
      0)
    return RPC_SRV_RESULT_HOST_NOT_REACHABLE_ERR;
  RPC_SRV_RESULT result = Client.get_string_type((char *)"get_my_public_ip",
                                                 (char *)"ip", temp_str);
  Client.rpc_server_disconnect();
  returnval = temp_str;
  return result;
...
/* ------------------------------------------------------------------------- */
RPC_SRV_RESULT XmppMgr::proc_cmd_fmw_get_localip(std::string msg,
                                                 std::string &returnval) {
  char temp_str[255];
  temp_str[0] = '\0';
  ADJsonRpcClient Client;
  ...
  if (XmppNetInterface ==
      "") // if not requested via cmdlinearg, then scan for available ones
  {
    result = Client.get_string_type_with_string_para(
        (char *)"get_ip_addr", (char *)"iface", (char *)"eth0", temp_str,
        (char *)"addr"); // first try for eth0
    if (result != RPC_SRV_RESULT_SUCCESS) {
      result = Client.get_string_type_with_string_para(
          (char *)"get_ip_addr", (char *)"iface", (char *)"eth1", temp_str,
          (char *)"addr"); // second try for eth1
      if (result != RPC_SRV_RESULT_SUCCESS) {
        result = Client.get_string_type_with_string_para(
            (char *)"get_ip_addr", (char *)"iface", (char *)"ppp0", temp_str,
            (char *)"addr"); // third try for ppp0
      }
    }
  } else {
    result = Client.get_string_type_with_string_para(
        (char *)"get_ip_addr", (char *)"iface",
        (char *)XmppNetInterface.c_str(), temp_str,
        (char *)"addr"); // check the requested iface name
  }
  Client.rpc_server_disconnect();
  returnval = temp_str;
  return result;
...
RPC_SRV_RESULT XmppMgr::proc_cmd_fmw_set_default_hostname(std::string msg) {
  ...
  if (Client.rpc_server_connect(bboxSmsServerAddr.c_str(), ADCMN_PORT_SYSMGR) !=
    ...
    return RPC_SRV_RESULT_HOST_NOT_REACHABLE_ERR;
  RPC_SRV_RESULT result =
      Client.set_action_noarg_type((char *)"set_default_hostname");
  Client.rpc_server_disconnect();
  return result;
}
/* ------------------------------------------------------------------------- */
RPC_SRV_RESULT XmppMgr::proc_cmd_dial_voice(std::string msg,
  ...
  if (Client.rpc_server_connect(bboxSmsServerAddr.c_str(),
    ...
    return RPC_SRV_RESULT_HOST_NOT_REACHABLE_ERR;
  RPC_SRV_RESULT result = Client.set_single_string_get_single_string_type(
      (char *)rpc_cmd, //"dial_voice",
      (char *)"destination", (char *)destArg.c_str(), (char *)"taskId",
      (char *)tID);
  Client.rpc_server_disconnect();
  returnval = "taskID="; // returnval+=tID;
  int xmptid = -1;
...
RPC_SRV_RESULT XmppMgr::proc_cmd_get_ussd(std::string msg,
  ...
  if (Client.rpc_server_connect(bboxSmsServerAddr.c_str(),
    ...
    return RPC_SRV_RESULT_HOST_NOT_REACHABLE_ERR;
  RPC_SRV_RESULT result =
      Client.get_string_type((char *)"get_ussd", (char *)"message", temp_str);
  Client.rpc_server_disconnect();
  returnval = temp_str;
  return result;
}
...
RPC_SRV_RESULT XmppMgr::proc_cmd_logsts(std::string msg,
  ...
  if (logArg.size() <= 0) {
    ...
    ADJsonRpcClient Client;
    if (Client.rpc_server_connect(bboxSmsServerAddr.c_str(),
                                  atoi(portArg.c_str())) != 0)
      return RPC_SRV_RESULT_HOST_NOT_REACHABLE_ERR;
    RPC_SRV_RESULT result =
        Client.get_string_type((char *)RPCMGR_RPC_DEBUG_LOG_GET,
    ...
    if (Client.rpc_server_connect(bboxSmsServerAddr.c_str(),
      ...
      return RPC_SRV_RESULT_HOST_NOT_REACHABLE_ERR;
    RPC_SRV_RESULT result = Client.set_single_string_type(
        (char *)RPCMGR_RPC_DEBUG_LOG_SET, (char *)RPCMGR_RPC_DEBUG_LOG_ARGSTS,
        (char *)logArg.c_str());
    Client.rpc_server_disconnect();
    return result;
  }
...
RPC_SRV_RESULT XmppMgr::proc_cmd_gsm_modem_identify(std::string msg,
  ...
  if (Client.rpc_server_connect(bboxSmsServerAddr.c_str(),
    ...
    return RPC_SRV_RESULT_HOST_NOT_REACHABLE_ERR;
  RPC_SRV_RESULT result = Client.set_action_noarg_get_single_string_type(
      (char *)"identify_dev", (char *)"taskId", tID);
  Client.rpc_server_disconnect();
  returnval = "taskID="; // returnval+=tID;
  int xmptid = -1;
  if (result == RPC_SRV_RESULT_IN_PROG)
...
RPC_SRV_RESULT XmppMgr::proc_cmd_log_list_update(std::string msg,
  ...
  if (Client.rpc_server_connect(bboxSmsServerAddr.c_str(), ADCMN_PORT_SYSMGR) !=
    ...
    return RPC_SRV_RESULT_HOST_NOT_REACHABLE_ERR;
  RPC_SRV_RESULT result = Client.set_action_noarg_get_single_string_type(
      (char *)"update_loglist", (char *)"taskId", tID);
  Client.rpc_server_disconnect();
  returnval = "taskID="; // returnval+=tID;
  int xmptid = -1;
  if (result == RPC_SRV_RESULT_IN_PROG)
...
RPC_SRV_RESULT XmppMgr::proc_cmd_log_get_count(std::string msg,
                                               std::string &returnval) {
  char temp_str[255];
  temp_str[0] = '\0';
  ADJsonRpcClient Client;
  if (Client.rpc_server_connect(bboxSmsServerAddr.c_str(), ADCMN_PORT_SYSMGR) !=
      0)
    return RPC_SRV_RESULT_HOST_NOT_REACHABLE_ERR;
  RPC_SRV_RESULT result = Client.get_integer_type((char *)"get_log_count",
                                                  (char *)"logcount", temp_str);
  Client.rpc_server_disconnect();
  returnval = temp_str;
  return result;
}
RPC_SRV_RESULT XmppMgr::proc_cmd_log_get_line(std::string msg,
                                              std::string &returnval) {
  std::string cmd, cmdArg;
  stringstream msgstream(msg);
  msgstream >> cmd;
  ...
  returnmsg[0] = '\0';
  RPC_SRV_RESULT result = Client.get_int_type_with_string_para(
      (char *)"get_log_line", (char *)"index", msgIndex, returnmsg,
      (char *)"logmsg");
  returnval = returnmsg;
  Client.rpc_server_disconnect();
  return result;
...
RPC_SRV_RESULT XmppMgr::proc_cmd_event_gsm(std::string msg, std::string sender,
  ...
  std::vector<EventSubscrEntry>::iterator it =
      find_if(myEventList.begin(), myEventList.end(),
              FindEventSubscrEntry(sender, 0, EXMPP_EVNT_GSM));
...
RPC_SRV_RESULT XmppMgr::proc_cmd_event_gpio(std::string msg, std::string sender,
  ...
  std::vector<EventSubscrEntry>::iterator it =
      find_if(myEventList.begin(), myEventList.end(),
              FindEventSubscrEntry(sender, io, EXMPP_EVNT_GPIO));
  ...
  } else // write status
  {
    ...
    if (it != myEventList.end()) // found entry, modify existing entry
    {
      ...
      if (cmdArgVal == "1") {
        sts = true;
        myEventList.push_back(
            EventSubscrEntry(sender, EXMPP_EVNT_GPIO, io, sts));
...
RPC_SRV_RESULT
XmppMgr::LoadEventSubscrList(std::string listFile,
...
RPC_SRV_RESULT XmppMgr::ExtendEventSubscrList(std::string listFile,
                                              std::string addr,
...
RPC_SRV_RESULT
XmppMgr::RewriteEventSubscrList(std::string listFile,
                                std::vector<EventSubscrEntry> *pList) {
  if (listFile.size() <= 0)
    return RPC_SRV_RESULT_FAIL;
  std::ofstream outfile;
  outfile.open(listFile, std::ofstream::out | std::ofstream::trunc);
  for (std::vector<EventSubscrEntry>::iterator it = pList->begin();
       it != pList->end(); ++it) {
    if ((*it).m_Status == true) {
...
/* ------------------------------------------------------------------------- */
RPC_SRV_RESULT XmppMgr::proc_cmd_alias(std::string msg,
                                       std::string &returnval) {
  // transform(msg.begin(), msg.end(), msg.begin(), ::tolower);//convert all
  // lower case
  if (msg == "Alias" ||
      msg ==
          "alias") // if it is just the "alias" print the content of AliasList
  {
    returnval = '\n'; //"";
...
RPC_SRV_RESULT XmppMgr::ExtendAliasList(std::string listFile, std::string key,
                                        std::string val) {
  if (listFile.size() <= 0)
    return RPC_SRV_RESULT_FAIL;
  std::ofstream outfile;
...
/* ------------------------------------------------------------------------- */
RPC_SRV_RESULT XmppMgr::proc_cmd_bot_name(std::string msg,
                                          std::string &returnval) {
  std::string cmd, cmdArg;
  stringstream msgstream(msg);
  msgstream >> cmd;
  ...
  if (cmdArg.size() <= 0) // get-hostname-cmd
  {
    ...
    if (XmppBotNameFilePath.size() > 0) {
      std::ofstream outfile;
      outfile.open(XmppBotNameFilePath,
...
/* ------------------------------------------------------------------------- */
RPC_SRV_RESULT XmppMgr::proc_cmd_buddy_list(std::string msg,
                                            std::string &returnval) {
  returnval = '\n';
...
RPC_SRV_RESULT XmppMgr::GpioEventCallback(int evntNum, int evntArg) {
  ...
  for (it = myEventList.begin(); it != myEventList.end(); it++) {
    ...
    if (pEntry.m_EvntType == EXMPP_EVNT_GPIO && pEntry.m_EvntArg == evntArg &&
        pEntry.m_Status == true)
      XmppProxy.SendMessageToBuddy(pEntry.subscriber, evntStr, "event");
  }
  return RPC_SRV_RESULT_SUCCESS;
}
/* ------------------------------------------------------------------------- */
RPC_SRV_RESULT XmppMgr::proc_cmd_shellcmd(std::string msg,
                                          std::string &returnval,
                                          std::string sender,
                                          EXMPP_CMD_TYPES cmdtype) {
  std::string cmd, destArg; //,msgArg;
  stringstream msgstream(msg);
  msgstream >> cmd;
  // msgstream >> destArg;
...
/* ------------------------------------------------------------------------- */
RPC_SRV_RESULT XmppMgr::proc_cmd_shellcmdresp(std::string msg,
                                              std::string &returnval,
                                              std::string sender) {
  // returnval='\n';
  // XmppProxy.get_buddy_list(returnval);
  std::string filename(SHELLCMD_RESP_FILE_PATH); // SHELLCMD_RESP_FILE_PATH
                                                 // declared in ADCommon.hpp
  std::ifstream in(filename, std::ios::in | std::ios::binary);
  if (in) {
    in.seekg(0, std::ios::end);
    unsigned int length = in.tellg();
    if (length > SHELLCMD_RESP_MAX_FILE_SIZE) // SHELLCMD_RESP_MAX_FILE_SIZE
                                              // declared in ADCommon.hpp
...
/* ------------------------------------------------------------------------- */
RPC_SRV_RESULT XmppMgr::proc_cmd_devident(std::string msg,
                                          std::string &returnval,
                                          std::string sender) {
  char tID[255];
  tID[254] = '\0';
  ...
  if (Client.rpc_server_connect(bboxSmsServerAddr.c_str(), ADCMN_PORT_SYSMGR) !=
    ...
    return RPC_SRV_RESULT_HOST_NOT_REACHABLE_ERR;
  RPC_SRV_RESULT result = Client.set_action_noarg_get_single_string_type(
      (char *)"device_identify", (char *)"taskId", tID);
  Client.rpc_server_disconnect();
  returnval = "taskID="; // returnval+=tID;
  int xmptid = -1;
  if (result == RPC_SRV_RESULT_IN_PROG)
...
/* ------------------------------------------------------------------------- */
RPC_SRV_RESULT XmppMgr::proc_cmd_xmpshutdown(std::string msg,
                                             std::string &returnval,
                                             std::string sender) {
  ...
  ADJsonRpcClient Client;
  if (Client.rpc_server_connect(bboxSmsServerAddr.c_str(),
                                ADCMN_PORT_XMPROXY) != 0)
    return RPC_SRV_RESULT_HOST_NOT_REACHABLE_ERR;
  RPC_SRV_RESULT result = Client.set_action_noarg_get_single_string_type(
      (char *)"trigger_shutdown", (char *)"taskId", tID);
  Client.rpc_server_disconnect();
  returnval = "taskID=";
  int xmptid = -1;
  if (result == RPC_SRV_RESULT_IN_PROG)
...
RPC_SRV_RESULT XmppMgr::proc_cmd_sonoff(std::string msg,
                                        std::string &returnval) {
  RPC_SRV_RESULT result;
  std::string cmd, cmdArg, cmdArg2;
  stringstream msgstream(msg);
...
/* ------------------------------------------------------------------------- */
RPC_SRV_RESULT XmppMgr::proc_cmd_disp_print(std::string msg,
                                            std::string &returnval) {
  std::string cmd, cmdArg, cmdArg2;
  stringstream msgstream(msg);
  msgstream >> cmd;
  ...
  if (cmdArg2.size() <= 0) // read printed message
  {
    ...
    ADJsonRpcClient Client;
    if (Client.rpc_server_connect(bboxSmsServerAddr.c_str(),
                                  ADCMN_PORT_SYSMGR) != 0) // ADCMN_PORT_DISPSRV
      return RPC_SRV_RESULT_HOST_NOT_REACHABLE_ERR;
    RPC_SRV_RESULT result = Client.get_integer_type_with_addr_para(
        (char *)"display_print", (char *)"line", gpioAddr, (char *)"msg",
        temp_str);
    Client.rpc_server_disconnect();
    returnval = temp_str;
    return result;
  } else // write gpio pin
  {
    ...
    if (Client.rpc_server_connect(bboxSmsServerAddr.c_str(),
                                  ADCMN_PORT_SYSMGR) != 0) // ADCMN_PORT_DISPSRV
      return RPC_SRV_RESULT_HOST_NOT_REACHABLE_ERR;
    RPC_SRV_RESULT result = Client.set_double_string_type(
        (char *)"display_print", (char *)"line", (char *)cmdArg.c_str(),
        (char *)"msg", (char *)cmdArg2.c_str());
    Client.rpc_server_disconnect();
    return result;
  }
}
/* ------------------------------------------------------------------------- */
RPC_SRV_RESULT XmppMgr::proc_cmd_disp_backlight(std::string msg,
                                                std::string &returnval) {
  std::string cmd, cmdArg;
  stringstream msgstream(msg);
  msgstream >> cmd;
...
/* ------------------------------------------------------------------------- */
RPC_SRV_RESULT XmppMgr::proc_cmd_get_display_backlight(std::string msg,
                                                       std::string &returnval) {
  // 14:52:09.440-->{ "jsonrpc": "2.0", "method": "display_backlight_get", "id":
  // 0 } 14:52:09.440<--{ "jsonrpc": "2.0", "result": { "return": "Success",
  // "status": "on" }, "id": 0 }
  char temp_str[255];
  temp_str[0] = '\0';
  ...
  if (Client.rpc_server_connect(bboxSmsServerAddr.c_str(), ADCMN_PORT_SYSMGR) !=
      0)
    return RPC_SRV_RESULT_HOST_NOT_REACHABLE_ERR;
  RPC_SRV_RESULT result = Client.get_string_type(
      (char *)"display_backlight_get", (char *)"status", temp_str);
  Client.rpc_server_disconnect();
  returnval = temp_str;
  return result;
}
...
RPC_SRV_RESULT XmppMgr::proc_cmd_set_display_backlight(std::string msg) {
  // 14:52:21.336-->{ "jsonrpc": "2.0", "method": "display_backlight_set",
  // "params": { "status": "off" }, "id": 0 } 14:52:21.336<--{ "jsonrpc": "2.0",
  // "result": { "return": "Success" }, "id": 0 }
  std::string cmd, cmdArg;
  stringstream msgstream(msg);
  msgstream >> cmd;
  msgstream >> cmdArg;
  ...
  if (Client.rpc_server_connect(bboxSmsServerAddr.c_str(), ADCMN_PORT_SYSMGR) !=
    ...
    return RPC_SRV_RESULT_HOST_NOT_REACHABLE_ERR;
  RPC_SRV_RESULT result =
      Client.set_single_string_type((char *)"display_backlight_set",
                                    (char *)"status", (char *)cmdArg.c_str());
  Client.rpc_server_disconnect();
  return result;
}
/* ------------------------------------------------------------------------- */
...
/* ------------------------------------------------------------------------- */
// split strings by space, expand string if prefixed with  $ sign and merge back
// the string
...
/* ------------------------------------------------------------------------- */
RPC_SRV_RESULT XmppMgr::proc_cmd_buddy_add(std::string msg,
                                           std::string &returnval,
                                           std::string sender) {
  ...
  if (cmdArg.size() <= 0) // user-id must be passed
    ...
    xpandarg(cmdArg); // e.g: addbuddy $name readonly //TODO: pass access level
                      // for this buddy.
    // remove white space in the beginning
    std::string::iterator end_pos =
        std::remove(cmdArg.begin(), cmdArg.end(), ' ');
...
/* ------------------------------------------------------------------------- */
RPC_SRV_RESULT XmppMgr::proc_cmd_buddy_remove(std::string msg,
                                              std::string &returnval,
                                              std::string sender) {
  // check if sender has the appropriate access for this command
  if (!XmppProxy.is_admin_user(sender))
...
RPC_SRV_RESULT XmppMgr::proc_cmd_send_message(std::string to,
  ...
  if (XmppProxy.IsMyBuddy(to) == false)
    return RPC_SRV_RESULT_ACTION_NOT_ALLOWED; // not my buddy, first subscribe
...
/* ------------------------------------------------------------------------- */
RPC_SRV_RESULT XmppMgr::proc_cmd_buddy_subscribe(std::string msg,
                                                 std::string &returnval,
                                                 std::string sender) {
  // check if sender has the appropriate access for this command
  if (!XmppProxy.is_admin_user(sender))
...
RPC_SRV_RESULT XmppMgr::proc_cmd_buddy_unsubscribe(std::string msg,
                                                   std::string &returnval,
                                                   std::string sender) {
  // check if sender has the appropriate access for this command
  if (!XmppProxy.is_admin_user(sender))
...
/* ------------------------------------------------------------------------- */
RPC_SRV_RESULT XmppMgr::proc_cmd_accept_buddy_list(std::string msg,
                                                   std::string &returnval) {
  returnval = '\n';
...
/* ------------------------------------------------------------------------- */
RPC_SRV_RESULT XmppMgr::proc_cmd_relay_message(std::string msg,
                                               std::string &returnval,
                                               std::string sender) {
  // if the request comes via xmpp channel, then allow only admin to run this
  // command but if this request comes via rpcclient, we cannot check if this
  // was called by admin
  if (!XmppProxy.is_admin_user(sender))
    return RPC_SRV_RESULT_ACTION_NOT_ALLOWED;
...
// client(admin)
RPC_SRV_RESULT XmppMgr::proc_cmd_subscribe_message(std::string to,
                                                   std::string message,
                                                   std::string subject) {
...
// this function can be called via local rpcclient or via external jabber
// client(admin)
RPC_SRV_RESULT XmppMgr::proc_cmd_add_buddy(std::string to, std::string message,
                                           std::string subject) {
