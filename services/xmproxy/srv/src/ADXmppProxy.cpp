...
int ADXmppProxy::send_reply(std::string reply, std::string sender) {
  ...
  if (it != mySessions.end()) {
    ...
    if (DebugLog)
      cout << "ADXmppProxy::send_reply:to:" << sender
           << " sending msg = " << reply << endl;
    return 0;
  }
  return -1;
...
int ADXmppProxy::monoshot_callback_function(void *pUserData,
  ...
  while (!PingPipe.empty()) {
    ...
    if (j != NULL) {
      ...
      {
        if (DebugLog)
          cout << "ADXmppProxy::send_client_alive_ping:going to disconnect"
               << endl;
        // j->disconnect();//ConnNotConnected);//ConnTlsFailed);// fake
        // disconnect reason so that no </stream:stream> is sent
        disconnect();
        if (DebugLog)
...
void ADXmppProxy::handleItemSubscribed(const JID &jid) {
  // printf( "subscribed %s\n", jid.bare().c_str() );
  if (DebugLog)
    cout << "ADXmppProxy::handleItemSubscribed: subscribed:"
         << jid.bare().c_str() << endl;
}
void ADXmppProxy::handleItemAdded(const JID &jid) {
  // printf( "added %s\n", jid.bare().c_str() );
  if (DebugLog)
    cout << "ADXmppProxy::handleItemAdded: added:" << jid.bare().c_str()
         << endl;
...
void ADXmppProxy::handleItemRemoved(const JID &jid) {
  // printf( "removed %s\n", jid.bare().c_str() );
  if (DebugLog)
    cout << "ADXmppProxy::handleItemRemoved: removed:" << jid.bare().c_str()
         << endl;
}
...
bool ADXmppProxy::SendMessageToBuddy(std::string address,
  ...
  // non-buddy - it will not be delivered) vector<std::string>::iterator it;
  ...
  for (; it != pRoster->end(); ++it) {
    ...
    if (address == str) // send message only if sender is in our Roster()
    {
      ...
      } else // if session doesnt exist in my list, then create new session and
             // send the message
      {
        MessageSession *session = new MessageSession(j, JID(address));
        session->send(
            body,
            subject); // todo: presence check needed before sending message?
...
bool ADXmppProxy::IsMyBuddy(std::string buddyaddress) {
  if (iConnect == false) {
    if (DebugLog)
      cout << "ADXmppProxy::IsMyBuddy:client disconnected, cannot check if its "
              "my buddy!!!"
           << endl;
    return false;
  }
