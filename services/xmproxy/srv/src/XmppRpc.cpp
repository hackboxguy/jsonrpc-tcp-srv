...
int XmppRpc::MapJsonToBinary(JsonDataCommObj *pReq, int index) {
  ...
  // json_to_bin_get_total_sms(pReq); case EJSON_BBOXSMS_RPC_SMS_GET :return
  // json_to_bin_get_sms(pReq); case EJSON_BBOXSMS_RPC_SMS_SEND        :break;
  // case EJSON_BBOXSMS_RPC_SMS_LIST_UPDATE :return
  // json_to_bin_sms_list_update(pReq); case
  // EJSON_BBOXSMS_RPC_SMS_IDENTIFY_DEV:return json_to_bin_ident_device(pReq);
  default:
    break;
  }
  return -1; // 0;
...
int XmppRpc::MapBinaryToJson(JsonDataCommObj *pReq, int index) {
  ...
  // case EJSON_BBOXSMS_RPC_SMS_TOTAL_GET   :return
  // bin_to_json_get_total_sms(pReq); case EJSON_BBOXSMS_RPC_SMS_GET :return
  // bin_to_json_get_sms(pReq); case EJSON_BBOXSMS_RPC_SMS_SEND        :break;
  // case EJSON_BBOXSMS_RPC_SMS_LIST_UPDATE :return
  // bin_to_json_sms_list_update(pReq); case
  // EJSON_BBOXSMS_RPC_SMS_IDENTIFY_DEV:return bin_to_json_ident_device(pReq);
  default:
    break;
  }
  return -1; // 0;
...
int XmppRpc::ProcessWork(JsonDataCommObj *pReq, int index,
  ...
  // case EJSON_BBOXSMS_RPC_SMS_LIST_UPDATE :return
  // process_sms_list_update(pReq,pObj); case
  // EJSON_BBOXSMS_RPC_SMS_IDENTIFY_DEV:return process_ident_device(pReq,pObj);
  default:
    break;
  }
  return 0;
...
XMPROXY_ASYNCTASK_TYPE XmppRpc::get_async_task_in_progress() {
  ...
  // :task=BBOXSMS_ASYNCTASK_LIST_UPDATE;break; case
  // EJSON_BBOXSMS_RPC_SMS_IDENTIFY_DEV:task=BBOXSMS_ASYNCTASK_IDENTIFY_DEV;break;
  // case EJSON_BBOXSMS_RPC_SMS_DELETE_ALL
  // :task=BBOXSMS_ASYNCTASK_DELETE_ALL;break;
  default:
    break;
  }
  return task;
...
int SmsRpc::process_delete_all(JsonDataCommObj* pReq,ADJsonRpcMgrProducer* pObj)
{
        ...
        OBJECT_MEM_NEW(pWorkData,BBOXSMS_SMS_PACKET);//delete this object in
run_work() callback function if(pWorkData == NULL)
        {
                pPanelReq->result=RPC_SRV_RESULT_MEM_ERROR;
                return -1;
        }
        //pWorkData->operation=pPacket->operation;
        pPanelReq->result=pObj->PushAsyncTask(EJSON_BBOXSMS_RPC_SMS_DELETE_ALL,(unsigned
char*)pWorkData,&pPacket->taskID,WORK_CMD_AFTER_DONE_PRESERVE);
...
int SmsRpc::process_sms_list_update(JsonDataCommObj* pReq,ADJsonRpcMgrProducer*
pObj)
{
        RPC_SRV_REQ *pPanelReq=NULL;
        pPanelReq=(RPC_SRV_REQ *)pReq->pDataObj;
...
run_work() callback function if(pWorkData == NULL)
        ...
        //pWorkData->operation=pPacket->operation;
        pPanelReq->result=pObj->PushAsyncTask(EJSON_BBOXSMS_RPC_SMS_LIST_UPDATE,(unsigned
char*)pWorkData,&pPacket->taskID,WORK_CMD_AFTER_DONE_DELETE);//WORK_CMD_AFTER_DONE_PRESERVE);
        if(pPanelReq->result!=RPC_SRV_RESULT_IN_PROG)
                OBJ_MEM_DELETE(pWorkData);
        pPanelReq->result=RPC_SRV_RESULT_SUCCESS;//although, this is async type,
but internally taskID object is auto-deleted after completion return 0;
}
...
int SmsRpc::process_ident_device(JsonDataCommObj* pReq,ADJsonRpcMgrProducer*
pObj)
{
        RPC_SRV_REQ *pPanelReq=NULL;
        pPanelReq=(RPC_SRV_REQ *)pReq->pDataObj;
...
run_work() callback function if(pWorkData == NULL)
        ...
        //pWorkData->operation=pPacket->operation;
        pPanelReq->result=pObj->PushAsyncTask(EJSON_BBOXSMS_RPC_SMS_IDENTIFY_DEV,(unsigned
char*)pWorkData,&pPacket->taskID,WORK_CMD_AFTER_DONE_PRESERVE);
        if(pPanelReq->result!=RPC_SRV_RESULT_IN_PROG)
                OBJ_MEM_DELETE(pWorkData);
        return 0;
