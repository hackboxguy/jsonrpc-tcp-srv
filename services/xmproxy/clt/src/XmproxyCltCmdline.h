...
class XmproxyCltCmdline : public ADCmdlineHelperConsumer,
  ...
  int push_send_msg_command(char *subarg, char *rpc_name, int rpc_index,
                            char *arg_name,
                            char *arg_name2); //,char* result_name);
  int run_send_msg_command(CmdExecutionObj *pCmdObj,
                           ADJsonRpcClient *pSrvSockConn,
                           ADGenericChain *pOutMsgList,
                           ADThreadedSockClientProducer *pWorker);
