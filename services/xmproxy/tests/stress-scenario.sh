...
cp "$HERE/fixtures/xmpp-alias-list.txt" "$HERE/fixtures/xmpp-botname.txt" "$RUN/"; : > "$RUN/xmpp-evntsubscr.txt"
...
"$INSTALL/bin/xmproxysrv" --syscfg=docker --port=40005 --loginfile="$HERE/fixtures/xmpp-login.txt" \
  ...
  --evntsubscr="$RUN/xmpp-evntsubscr.txt" --iface=lo >"$RUN/xmproxysrv.log" 2>&1 & echo $! > "$RUN/xmproxysrv.pid"
