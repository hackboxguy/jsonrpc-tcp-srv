...
// pin RPI_GPIO_PIN_11) You can do some safe, non-destructive testing on any
// platform with: gcc bcm2835.c -D BCM2835_TEST
// ./a.out
// #define BCM2835_TEST
...
// access Instead it prints out what it _would_ do if debug were 0
...
uint8_t bcm2835_spi_transfer(uint8_t value) {
  ...
  // else accesses a different peripheral? Clear TX and RX fifos
...
void bcm2835_spi_transfernb(char *tbuf, char *rbuf, uint32_t len) {
  ...
  // This is Polled transfer as per section 10.6.1
  // BUG ALERT: what happens if we get interupted in this section, and someone
  // else accesses a different peripheral?
...
void bcm2835_spi_writenb(char *tbuf, uint32_t len) {
  ...
  // This is Polled transfer as per section 10.6.1
  // BUG ALERT: what happens if we get interrupted in this section, and someone
  // else accesses a different peripheral?
...
#else
  if ((fd = open(bcm2835_get_pi_version() == 1 ? "/dev/i2c-0" : "/dev/i2c-1",
                 O_RDWR)) < 0)
#endif
    return fd;
...
int bcm2835_i2c_write(const char *buf, uint32_t len) {
  ...
  // Write bulk data
    ...
    // reason = i2c_smbus_write_i2c_block_data(i2c_fd, buf[0], len-1, (const
    // __u8 *) &buf[1]);
    union i2c_smbus_data data;
    int i;
