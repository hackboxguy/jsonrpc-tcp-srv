...
//              Added option to use custom Chip Select Pin PI GPIO instead of
//              only CE0 CE1 Done a hack to use CE1 by software as custom CS pin
//              because HW does not work Added function to determine PI revision
//              board
//							Added function to set
// SPI speed (instead of divider for easier look in code)
...
/// 26 pin IDE plug on the RPi board so you can control and interface with
/// various external devices.
///
/// It provides functions for reading digital inputs and setting digital
/// outputs, using SPI and I2C, and for accessing the system timers. Pin event
/// detection is supported by polling (interrupts are not supported).
///
/// It is C++ compatible, and installs as a header file and non-shared library
/// on any Linux-based distro (but clearly is no use except on Raspberry Pi or
/// another board with BCM 2835).
///
/// The version of the package that this documentation refers to can be
/// downloaded from http://www.airspayce.com/mikem/bcm2835/bcm2835-1.25.tar.gz
...
/// http://groups.google.com/group/bcm2835 Please use that group for all
/// questions and discussions on this topic. Do not contact the author directly,
/// unless it is to discuss commercial licensing.
///
...
/// hangs on 2012-07-15-wheezy-raspbian and Occidentalisv01. Reason for this is
/// not yet determined, but suspect that an interrupt handler is hitting a hard
/// loop on those OSs. If you must use bcm2835_gpio_len() and friends, make sure
/// you disable the pins with bcm2835_gpio_cler_len() and friends after use.
///
/// \par Installation
///
...
/// They are designed to use physical addresses as described in section 1.2.3
/// ARM physical addresses of the BCM2835 ARM Peripherals manual. Physical
...
/// range starting at 0x7E000000. Thus a peripheral advertised in the manual at
/// bus address 0x7Ennnnnn is available at physical address 0x20nnnnnn.
///
/// The base address of the various peripheral registers are available with the
/// following externals: bcm2835_gpio bcm2835_pwm bcm2835_clk bcm2835_pads
/// bcm2835_spio0
/// bcm2835_st
/// bcm2835_bsc0
/// bcm2835_bsc1
...
///
/// The functions in this library are designed to be passed the BCM 2835 GPIO
/// pin number and _not_ the RPi pin number. There are symbolic definitions for
/// each of the available pins that you should use for convenience. See \ref
/// RPiGPIOPin.
///
/// \par SPI Pins
///
/// The bcm2835_spi_* functions allow you to control the BCM 2835 SPI0
/// interface, allowing you to send and received data by SPI (Serial Peripheral
/// Interface). For more information about SPI, see
...
/// While SPI is in use, you will not be able to control the state of the SPI
/// pins through the usual bcm2835_spi_gpio_write(). When bcm2835_spi_end() is
/// called, the SPI pins will all revert to inputs, and can then be configured
/// and controled with the usual bcm2835_gpio_* calls.
///
...
/// allowing you to send and received data by I2C ("eye-squared cee";
/// generically referred to as "two-wire interface") . For more information
/// about I2C, see http://en.wikipedia.org/wiki/I%C2%B2C
///
...
/// source community in accordance with the GPL Version 2 when your application
/// is distributed. See http://www.gnu.org/copyleft/gpl.html and COPYING
///
...
/// David Robinson. \version 1.5 Added bcm2835_close() to deinit the library.
/// Suggested by C?sar Ortiz \version 1.6 Document testing on
/// 2012-07-15-wheezy-raspbian and Occidentalisv01
///              Functions bcm2835_gpio_ren(), bcm2835_gpio_fen(),
///              bcm2835_gpio_hen()
///               bcm2835_gpio_len(), bcm2835_gpio_aren() and
///               bcm2835_gpio_afen() now changes only the pin specified. Other
///               pins that were already previously enabled stay enabled.
///              Added  bcm2835_gpio_clr_ren(), bcm2835_gpio_clr_fen(),
///              bcm2835_gpio_clr_hen()
///                bcm2835_gpio_clr_len(), bcm2835_gpio_clr_aren(),
///                bcm2835_gpio_clr_afen() to clear the enable for individual
///                pins, suggested by Andreas Sundstrom.
/// \version 1.7 Added bcm2835_spi_transfernb to support different buffers for
/// read and write. \version 1.8 Improvements to read barrier, as suggested by
/// maddin. \version 1.9 Improvements contributed by mikew:
///              I noticed that it was mallocing memory for the mmaps on
///              /dev/mem. It's not necessary to do that, you can just mmap the
///              file directly, so I've removed the mallocs (and frees). I've
///              also modified delayMicroseconds() to use nanosleep() for long
///              waits, and a busy wait on a high resolution timer for the rest.
///              This is because I've found that calling nanosleep() takes at
///              least 100-200 us. You need to link using '-lrt' using this
///              version. I've added some unsigned casts to the debug prints to
///              silence compiler warnings I was getting, fixed some typos, and
///              changed the value of BCM2835_PAD_HYSTERESIS_ENABLED to 0x08 as
///              per Gert van Loo's doc at
///              http://www.scribd.com/doc/101830961/GPIO-Pads-Control2
///              Also added a define for the passwrd value that Gert says is
///              needed to change pad control settings.
/// \version 1.10 Changed the names of the delay functions to bcm2835_delay()
///              and bcm2835_delayMicroseconds() to prevent collisions with
///              wiringPi. Macros to map delay()-> bcm2835_delay() and Macros to
///              map delayMicroseconds()-> bcm2835_delayMicroseconds(), which
...
/// different GPIO mapping) \version 1.13 New GPIO pin definitions for RPi
/// version 2 plug P5
///               Hardware base pointers are now available (after
///               initialisation) externally as bcm2835_gpio bcm2835_pwm
///               bcm2835_clk bcm2835_pads bcm2835_spi0.
/// \version 1.14 Now compiles even if CLOCK_MONOTONIC_RAW is not available,
/// uses CLOCK_MONOTONIC instead.
///               Fixed errors in documentation of SPI divider frequencies based
///               on 250MHz clock. Reported by Ben Simpson.
/// \version 1.15 Added bcm2835_close() to end of examples as suggested by Mark
/// Wolfe. \version 1.16 Added bcm2835_gpio_set_multi, bcm2835_gpio_clr_multi
/// and bcm2835_gpio_write_multi
///               to allow a mask of pins to be set all at once. Requested by
///               Sebastian Loncar.
...
///               now uses the RPi system timer counter, instead of
///               clock_gettime, for improved accuracy. No need to link with
///               -lrt now. Contributed by Arjan van Vught.
/// \version 1.19 Removed inlines added by previous patch since they don't seem
/// to work everywhere.
///               Reported by olly.
/// \version 1.20 Patch from Mark Dootson to close /dev/mem after access to the
/// peripherals has been granted. \version 1.21 delayMicroseconds is now not
/// susceptible to 32 bit timer overruns.
...
/// \version 1.23 Added bcm2835_i2c_set_baudrate and
/// bcm2835_i2c_read_register_rs.
///               Improvements to bcm2835_i2c_read and bcm2835_i2c_write
///               functions to fix ocasional reads not completing. Patched by
///               Mark Dootson.
...
/// airspayce.com \author  Mike McCauley (mikem@airspayce.com) DO NOT CONTACT
/// THE AUTHOR DIRECTLY: USE THE LISTS
...
/// The values here are designed to be passed to various functions in the
/// bcm2835 library.
/// @{
...
/// GPIO register offsets from BCM2835_GPIO_BASE. Offsets into the GPIO
/// Peripheral block in bytes per 6.1 Register View
...
/// requiring a pin. Not all pins on the RPi 26 bin IDE plug are connected to
/// GPIO pins and some can adopt an alternate function. RPi version 2 has some
/// slightly different pinouts, and these are values RPI_V2_*. At bootup, pins 8
/// and 10 are set to UART0_TXD, UART0_RXD (ie the alt0 function) respectively
/// When SPI0 is in use (ie after bcm2835_spi_begin()), pins 19, 21, 23, 24, 26
/// are dedicated to SPI and cant be controlled independently
typedef enum {
...
typedef enum {
  ...
  // Only GPIO > 3 can be used (to not interfere with the previous value just
  // above ) Lucky we have plenty of theese pins
...
/// It is reported that (contrary to the documentation) any even divider may
/// used. The frequencies shown for each divider have been confirmed by
/// measurement
typedef enum {
  ...
  BCM2835_SPI_CLOCK_DIVIDER_1 =
      1, ///< 0 = 262.144us = 3.814697260kHz, same as 0/65536
...
/// bcm2835_set_debug). If bcm2835_init() fails by returning 0, calling any
/// other function may result in crashes or other failures. Prints messages to
/// stderr in case of errors. \return 1 if successful else 0
...
/// bcm2835_peri_read() within the same peripheral, and no other peripheral
/// access has occurred since. \param[in] paddr Physical address to read from.
/// See BCM2835_GPIO_BASE etc. \return the value read from the 32 bit register
/// \sa Physical Addresses
...
/// You should only use this when your code has previously called
/// bcm2835_peri_write() within the same peripheral, and no other peripheral
/// access has occurred since. \param[in] paddr Physical address to read from.
/// See BCM2835_GPIO_BASE etc. \param[in] value The 32 bit value to write \sa
/// Physical Addresses
...
/// RPiGPIOPin. \param[in] on HIGH to clock the value from bcm2835_gpio_pud()
/// into the pin. LOW to remove the clock. \sa bcm2835_gpio_set_pud()
...
/// However, you are at the mercy of nanosleep(). From the manual for
/// nanosleep(): If the interval specified in req is not an exact multiple of
/// the granularity underlying  clock  (see  time(7)),  then the interval will
/// be rounded up to the next multiple. Furthermore, after the sleep completes,
...
/// by value \param[in] value values required for each bit masked in by mask,
/// eg: (1 << RPI_GPIO_P1_03) | (1 << RPI_GPIO_P1_05) \param[in] mask Mask of
/// pins to affect. Use eg: (1 << RPI_GPIO_P1_03) | (1 << RPI_GPIO_P1_05)
...
/// and P1-26 (CE1) to alternate function ALT0, which enables those pins for SPI
/// interface. \param[in] cs Specifies the CS pins(s) that are used to activate
/// the desired slave.
///   One of BCM2835_SPI_CS*, see \ref bcm2835SPIChipSelect
///   or one of One of BCM2835_SPI_CS*, see \ref bcm2835SPIChipSelect
///   by default PI hardware driven using CE0
/// You should call bcm2835_spi_end() when all SPI funcitons are complete to
/// return the pins to their default functions \sa  bcm2835_spi_end()
extern void bcm2835_spi_begin(uint8_t cs);

/// End SPI operations.
/// SPI0 pins P1-19 (MOSI), P1-21 (MISO), P1-23 (CLK), P1-24 (CE0) and P1-26
/// (CE1) are returned to their default INPUT behaviour.
...
/// MOSI, and simultaneously clocks in data from MISO. Returns the read data
/// byte from the slave. Uses polled transfer as per section 10.6.1 of the BCM
/// 2835 ARM Peripherls manual \param[in] value The 8 bit data byte to write to
/// MOSI \return The 8 bit byte simultaneously read from  MISO \sa
/// bcm2835_spi_transfern()
extern uint8_t bcm2835_spi_transfer(uint8_t value);

/// Transfers any number of bytes to and from the currently selected SPI slave.
/// Asserts the currently selected CS pins (as previously set by
/// bcm2835_spi_chipSelect) during the transfer. Clocks the len 8 bit bytes out
/// on MOSI, and simultaneously clocks in data from MISO. The data read read
...
/// manual \param[in] tbuf Buffer of bytes to send. \param[out] rbuf Received
/// bytes will by put in this buffer \param[in] len Number of bytes in the tbuf
/// buffer, and the number of bytes to send/received \sa bcm2835_spi_transfer()
...
/// replace the contents \param[in] len Number of bytes int eh buffer, and the
/// number of bytes to send/received \sa bcm2835_spi_transfer()
extern void bcm2835_spi_transfern(char *buf, uint32_t len);

/// Transfers any number of bytes to the currently selected SPI slave.
/// Asserts the currently selected CS pins (as previously set by
/// bcm2835_spi_chipSelect) during the transfer. \param[in] buf Buffer of bytes
/// to send. \param[in] len Number of bytes in the tbuf buffer, and the number
/// of bytes to send
...
/// return the pins to their default functions \sa  bcm2835_i2c_end()
