  ...
  while (!NotifierList.empty()) {
    ...
    if (Client.rpc_server_connect("127.0.0.1", entry.port) != 0) {
      LOG_ERR_MSG_WITH_ARG("libadav:ADEvntNotifier",
                           "Unable connect on port = %d", entry.port);
    } else {
      if (Client.set_three_int_type(
              ...
              entry.eventArg2) != RPC_SRV_RESULT_SUCCESS) {
        LOG_ERR_MSG_WITH_ARG("libadav:ADEvntNotifier", "Unable send event = %d",
                             entry.eventNum);
        LOG_ERR_MSG_WITH_ARG("libadav:ADEvntNotifier",
                             "Unable send event on port = %d", entry.port);
      }
